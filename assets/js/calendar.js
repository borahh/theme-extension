...
const sortData = () => {
  const InJSON = (dates) => {
    ...
    dates.forEach((item) => {
      ...
      if (
        sortedDates.at(-1).year === year &&
        sortedDates.at(-1).month === month
      ) {
  ...
  const optionalDates = document
    .querySelector("#property-availability")
    .getAttribute("data-option-dates")
    .split(",");
  const sortedOptionalDates = InJSON(optionalDates);

  // get blocked dates in json format
  const blockedDates = document
    .querySelector("#property-availability")
    .getAttribute("data-blocked-dates")
    .split(",");
  const sortedBlockedDates = InJSON(blockedDates);

  //get checkOutDates
  const checkOutDates = document
    .querySelector("#property-availability")
    .getAttribute("data-out-dates")
    .split(",");
  const sortedCheckOutDates = InJSON(checkOutDates);
  // getCheckInDates
  const checkInDates = document
    .querySelector("#property-availability")
    .getAttribute("data-in-dates")
    .split(",");
...
const markOnCalendar = () => {
  ...
  calenders.forEach((calender) => {
    const [month, year] = calender
      .querySelector(".month-name")
      .innerText.split(",");

    sortedOptionalDates.forEach((item) => {
      if (
        item.year == year.replace(" ", "") &&
        parseInt(item.month) == monthNumber.indexOf(month.toLowerCase()) + 1
      ) {
...
const markOnDatePicker = () => {
  const calenders = document.querySelectorAll(
...
async function calculateCost(startDate, endDate, flag) {
  ...
  const serviceCharges = parseFloat(
    document.querySelector(".service-charges").value
  );
  ...
  let days = parseInt(
    (endDate.split("-")[2] - startDate.split("-")[2]) / 1000 / 60 / 60 / 24
  );
  ...
  if ("seasonal" === propertyPricingType) {
    ...
    fetch(
      "https://icvillastbarth.com/wp-admin/admin-ajax.php",
      fetchStayingNightsCost
    )
      .then((res) => res.json)
  ...
  console.log(costStayingNights, serviceCharges);
  ...
  const extraGuestsNum =
    guestsNum - guestsCapacity > 0 ? guestsNum - guestsCapacity : 0;
  const extraGuests = document.querySelector(".extra-guests").value;
  const perExtraGuestPrice = parseInt(
    document.querySelector(".per-extra-guest-price").value
  );
  ...
  if (additionalFeesFields) {
    ...
    const eventHandler = function () {
      // Redo the calculations on adults change.
    ...
    if ("adult" === book_child_as) {
      // Check if child will be booked as an adult.
  ...
  fetch(document.querySelector(".rvr-booking-form").getAttribute("action"), {
    ...
    .then((res) => res.json())
    .then(function (response) {
      // Set prices with their other relevant data to the calculation table and then display the table.
      ...
      const snField = document.querySelector(
        ".staying-nights-count-field .cost-value"
      );
      const psnField = document.querySelector(
        ".staying-nights-field .cost-value"
      );
      const scField = document.querySelector(
        ".services-charges-field .cost-value"
      );
      const stField = document.querySelector(
        ".subtotal-price-field .cost-value"
      );
      ...
      if (additionalFees) {
        additionalFees.forEach(function (field) {
          let feeCalcField = document
            .querySelector("." + field["name"] + "-fee-field")
...
window.addEventListener("load", () => {
  ...
  //  room filter
  const pricesTableRows = document.querySelectorAll(
  ...
  //   NavBar
  document
    .querySelector(".rh_rvr_optional_services_wrapper")
    .setAttribute("id", "servies");
  document
    .querySelector(".property-detail-slider-wrapper")
    .setAttribute("id", "gallary");
