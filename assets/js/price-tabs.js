...
function openYear(evt, year) {
    ...
    var yearRoomElements = document.querySelectorAll(".year-" + year + '[data-room="' + selectedRoom + '"]');
    ...
    var type = document.querySelector('.day-week-tablinks.active').classList.contains('per-day') ? 'per-day' : 'per-week';
...
document.addEventListener('change', function(event) {
    ...
    if (event.target.matches('#rvr-guests')) {
        ...
        if (activeTab) {
            ...
            var type = activeTab.classList.contains('per-day') ? 'per-day' : 'per-week';
...
function showDayOrWeek(type) {
    ...
    for (var i = 0; i < perDayColumns.length; i++) {
        if (perDayColumns[i].parentNode.style.display !== 'none') {
            perDayColumns[i].style.display = type === 'per-day' ? "table-cell" : "none";
        }
    }
    for (var i = 0; i < perWeekColumns.length; i++) {
        if (perWeekColumns[i].parentNode.style.display !== 'none') {
            perWeekColumns[i].style.display = type === 'per-week' ? "table-cell" : "none";
        }
    }
...
window.onload = function() {
    ...
    var currentYearTablink = document.querySelector('.tablinks.year-' + currentYear);
